<template>
    ...
    Корзина
    <p v-if="cart.length !== 0">общая стоимость товаров: {{ totalPrice }}</p>
    <button v-if="cart.length !== 0" @click="goToCheckout" class="btn btn-dark">Оформить заказ</button>

    <div v-if="cart.length === 0"> В корзине пока что нет товаров...</div>
    <div v-if="cartFullData">
        <div v-for="cartItem in cartFullData" :key="cartItem.product.slug"
            ...
            query: {flower_count: cartItem.flower_count}
                ...
                <p style="color: #0a53be">название: {{ cartItem.product.name }}
                    {{ cartItem.flower_count ? '( ' + cartItem.flower_count + ' шт. )' : '' }}</p>
            </router-link>
            <button @click="increaseProductCount(cartItem)" class="btn btn-dark">+</button>
            <div>количество: {{ getCartItemCount(cartItem) }}</div>
            <button @click="decreaseProductCount(cartItem)" :disabled="getCartItemCount(cartItem) === 1"
                    class="btn btn-dark">-
            </button>
